class Ghost {
    ...
    constructor(el) {
      this.scene = el;
      this.clone = el.cloneNode(true);	//复制当前节点的所有子孙节点。
    ...
    // 嘴
    openMouth() {
      this.mouth.classList.remove('close');
    ...
    closeMouth() {
      ...
      this.mouth.classList.add('close');
    ...
    startActivity() {
      this.activityInterval = setInterval(() => {
		  this.neutralMouth();
		  setTimeout(() => {this.openMouth()},400)
		  setTimeout(() => {this.closeMouth()},800)
		  setTimeout(() => {this.neutralMouth()},1300)
